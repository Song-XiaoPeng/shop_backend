    ...
    <el-dialog
      ...
      <el-form :model="form1"
        ...
        <el-form-item label="分配提示" label-width="150px">
          ...
          <br/>
          成交情况 : {{form1.sell_buy + form1.bargain_amount + '股, 花费' + form1.bargain_price + '元'}}
        </el-form-item>
        <el-form-item label="子账户委托ID" label-width="150px" required prop="stock_finance_entrust_id">
          <el-input size="small" style="width:200px" v-model="form1.stock_finance_entrust_id"></el-input>
        </el-form-item>
      </el-form>
...
<script>
  ...
  import {assignBuyDayMakeDeal, assignBuyHistoryMakeDeal} from '../../../api/java';
    ...
    data() {
      return {
        ...
        excel_column_arr: ['parent_stock_finance_id', 'stock_code', 'stock_name', 'sell_buy',
          'parent_entrust_deal_time', 'parent_entrust_status', 'bargain_average_price', 'bargain_amount', 'bargain_price', 'parent_stock_finance_id',
          'makedeal_ids'
        ],
        ...
        status_list: [{value: 1, label: '今日母账户待系统补买'}, {value: 2, label: '历史母账户待系统补买'}],
        loading: false,
        todayOrHistory: 1,
        forms: {},
        form1: {client: ''},
        ...
        post_data: {
          field: ['id', 'parent_entrust_status', 'sell_buy', 'parent_finance_entrust_id', 'stock_code', 'stock_name',
            'bargain_average_price', 'entrust_time', 'bargain_amount', 'bargain_price',
            'parent_stock_finance_id', 'parent_entrust_deal_time', '"" as makedeal_ids'],
          search: {
            stock_code: '',
            stock_name: '',
          },
        ...
        this.$refs.Form1.validate((valid) => {
          if (valid) {
            this.$confirm('确认要分配吗?', '提示', {
              ...
              if (this.todayOrHistory == 1) {
                assignBuyDayMakeDeal({id: row.id, stockFinanceEntrustId: row.stock_finance_entrust_id}).then((res) => {
                  this.dialog1Visible = false;
                  this.ezNotifyAxiosThen(res);
                  this.getList();
                }).catch(error => this.ezNotifyAxiosCatch(error));
              } else {
                assignBuyHistoryMakeDeal({id: row.id, stockFinanceEntrustId: row.stock_finance_entrust_id}).then((res) => {
                  this.dialog1Visible = false;
                  this.ezNotifyAxiosThen(res);
                  this.getList();
                }).catch(error => this.ezNotifyAxiosCatch(error));
